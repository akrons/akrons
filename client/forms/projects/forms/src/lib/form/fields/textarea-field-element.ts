...
import { TEXTAREA_TYPE } from './entity-types';
...
export class TextareaField implements IFormElementDefinition {

    public id: string;
    public type = TEXTAREA_TYPE;
    ...
    static addFunctionality(functionality: Functionality): void {
        const scopeType = TEXTAREA_TYPE + '-field';
        functionality.addFunctions({
            scopeType,
            name: 'default',
            eval: (scope: TextareaField, parameters) => ({
                type: 'string',
                value: scope.value,
            }),
        });
