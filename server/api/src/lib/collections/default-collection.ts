...
export abstract class DefaultCollection<T extends IHaseId> {
    ...
    async create(id?: string, newElement?: T): Promise<T> {
        if (!id) {
    ...
    async update(id?: string, updated?: T): Promise<void> {
        if (!id) {
            if (!this.allowRandomId) {
                throw new BadRequestError();
            }
    ...
    abstract canMoveId(id: string, x: T): Promise<boolean>;
    abstract updateMap(x: T): Partial<T>;
    abstract insertMap(id: string, x: T): T;
